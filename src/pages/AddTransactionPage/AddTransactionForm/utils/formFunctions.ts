import { IAccount, ICategory, ITransaction, ITransactionType } from 'utils/interfaces';
...
export const submitForm = async (
  ...
  addNewtransferTransaction: (transactionTo: ITransaction, transactionFrom: ITransaction) => void,
  updateTransaction: (transaction: ITransaction, showAlert?: boolean) => void,
) => {
  ...
  if (isIncomeExpenseTransaction) {
    ...
    if (isUpdateTransaction) {
      updateTransaction({ ...transaction, id: currentTransaction?.id });
    } else {
    ...
    if (currentTransaction) {
      updateTransaction( { ...transactionFrom, id: transactionIDFrom }, false);
      updateTransaction( { ...transactionTo, id: transactionIDTo });
    } else {
      addNewtransferTransaction(transactionFrom, transactionTo);
    }
  }
};
